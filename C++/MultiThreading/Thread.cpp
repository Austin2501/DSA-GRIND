...
// Find the addition of all odd number from 1 to 1900000000 and all even numbers fromm 1 to 1900000000
...
int main(){
    ...
    // creating thread using function pointers
    thread t1(findEven, start, end);
    thread t2(findOdd, start, end);

    t1.join(); // it will join your thread with main() else threads will be running 
    t2.join();
...
// this program will run in terminal using command
// g++ -std=c++20  Thread.cpp -o Thread
// ./Thread